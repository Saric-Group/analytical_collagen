...
This is a main function of the code that automizes the creation of topology files, LAMMPS input scripts, and bash scripts to run the simulations on a remote cluster. For usage on a local installation of LAMMPS, some parts of the created scripts need to be adjusted manually or one can entirely skip using the scripts and just the topology file, for instance.
